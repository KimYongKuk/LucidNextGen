...
type GroupedChats = {
  pinned: Chat[];
  autoGenerated: Chat[]; // Phase 3b: cron 자동 생성 세션 (시간 그룹과 분리)
  today: Chat[];
  past: Chat[];
};
...
const groupChatsByDate = (chats: Chat[]): GroupedChats => {
  return chats.reduce(
    (groups, chat) => {
      ...
      // auto_generated는 truthy(1 또는 true)만 인정
      ...
      // 자동 생성 세션은 별도 그룹 — 일반 시간 그룹과 섞이지 않게
      if (isAuto) {
        groups.autoGenerated.push(chat);
        return groups;
      }
        ...
        <SidebarGroupContent>
          <SidebarMenu>
            <div className="flex flex-col gap-6">
              ...
              {groupedChats.autoGenerated.length > 0 && (
                <div>
                  <div className="px-2 py-1 text-sidebar-foreground/50 text-xs">
                    <b className="text-blue-600 dark:text-blue-400">🔄 자동 실행</b>
                  </div>
                  {groupedChats.autoGenerated.map((chat) => {
                    const chatId = chat.id || chat.session_id || "";
                    return (
                      <ChatItem
                        chat={chat}
