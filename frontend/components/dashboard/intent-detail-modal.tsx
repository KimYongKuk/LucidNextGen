...
import { reportApi, type IntentDetailData } from "@/lib/api/report"
...
export function IntentDetailModal({ intentKey, intentName, dateFrom, dateTo, onClose }: Props) {
  const [data, setData] = useState<IntentDetailData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    reportApi.getIntentDetail(dateFrom, dateTo, intentKey)
      .then(setData)
      .catch(() => setData(null))
      .finally(() => setLoading(false))
  }, [intentKey, dateFrom, dateTo])
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="relative mx-4 max-h-[80vh] w-full max-w-4xl overflow-hidden rounded-2xl border border-[#334155] bg-[#1F2937] shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#334155] px-6 py-4">
          <div>
            <h2 className="text-lg font-semibold text-[#F3F4F6]">{intentName}</h2>
                ...
                <tbody>
                  {data.messages.map((msg, i) => (
                    <tr key={i} className="border-b border-[#334155]/50 transition-colors hover:bg-[#334155]/20">
                      ...
                      <td className="whitespace-nowrap px-4 py-2.5 font-mono text-xs text-[#F3F4F6]">{msg.userId}</td>
                      <td className="max-w-[200px] truncate px-4 py-2.5 text-[#F3F4F6]" title={msg.question}>{msg.question}</td>
                      <td className="max-w-[250px] truncate px-4 py-2.5 text-[#9CA3AF]" title={msg.answer}>{msg.answer}</td>
