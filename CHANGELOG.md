...
## [2026-04-28]
- **수정** [ServiceMenu/겸직자] 그룹웨어 서비스 메뉴 위젯이 겸직자(예: A2208002 황기연 — 엘앤에프 공정기술파트 + 엘앤에프플러스 LFP공정운영팀)의 회사를 비결정적으로 한 개만 잡아 메뉴가 누락되던 이슈 수정. 원인은 `service_menu.py`의 `pool.fetchrow(... LIMIT 1)` + `ORDER BY` 없음 → PostgreSQL 반환 순서에 따라 회사가 임의로 결정 + `_company_cache`가 프로세스 수명 동안 박혀 재기동 전까지 한쪽 회사로 고정. `_get_company_name()` → `_get_company_names()`로 전환하여 `pool.fetch(...)`로 모든 부서 행 수집, `_resolve_company()` 헬퍼 분리, 중복 제거된 회사명 list 캐싱(`dict[str, list[str]]`). 엔드포인트 필터를 `if company_name in orgs` → `if company_set.intersection(orgs)`로 변경하여 OR(합집합) 시맨틱 적용. 정책: "어느 한쪽 회사라도 속하면 보여준다" — 엘앤에프 전용 메뉴는 엘앤에프 소속이 있는 사용자만(단독이든 겸직이든), JH화학공업 단독은 어떤 메뉴도 노출 안 됨, LFP+엘앤에프 겸직자는 양쪽 메뉴 모두 표시. 응답 필드 `company` → `companies` (list, 프론트 lucid-service-menu.js는 `data.menus`만 사용 → 호환성 영향 없음). → [상세](docs/history/2026-04-28_ServiceMenu_겸직자_지원.md)
