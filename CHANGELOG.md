...
## [2026-04-24]
- **수정** [Calendar/Embed] 그룹웨어 위젯 경로에서 GOSSOcookie가 백엔드까지 전달되지 않아 `create_event`가 "서버에 연결할 수 없습니다"로 실패하던 이슈 수정 — 위젯 JS는 그룹웨어 origin의 `document.cookie`에서 `GOSSOcookie`를 추출해 iframe src에 `?gosso=` 파라미터로 붙여줌. 그러나 `middleware.ts` matcher가 `/embed` 경로를 제외하므로 URL의 `gosso` 값을 `gosso` 쿠키로 이식하는 기존 로직이 iframe에서는 실행되지 않았고, `/embed/gw/page.tsx`도 `token/empno/sid`만 추출하고 `gosso`는 무시하여 `use-simple-chat.ts`의 `document.cookie` 기반 폴백조차 빈 문자열을 받음 → 백엔드가 서비스 계정 폴백으로만 POST하다 LFON 권한 거부로 실패(A2604011 재현, A2304013은 서비스 계정 폴백이 우연히 통과). `/embed/gw/page.tsx` 마운트 시점에 URL의 `gosso`를 읽어 `document.cookie`에 `gosso=...; SameSite=Lax; max-age=86400`으로 심어주도록 `useEffect` 추가. 이후 `use-simple-chat`이 기존 폴백 로직([use-simple-chat.ts:183](frontend/hooks/use-simple-chat.ts#L183))으로 자동 픽업하여 chat API body의 `gosso_cookie` 필드에 실림 → CalendarWorker가 사용자 본인 쿠키로 `/api/calendar/{id}/event/` POST 수행 → [상세](docs/history/2026-04-24_GW위젯_GOSSO_전달_수정.md)
