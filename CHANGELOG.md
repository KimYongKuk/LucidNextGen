...
## [2026-04-28]
- **수정** [Mail/JsonParse] 메일 MCP의 JSON strict 파싱이 JSP가 본문에 넣은 raw control char(`\n`/`\t`)를 거부하여 그룹웨어 임베드에서 "안 읽은 메일 확인해줘" 요청이 "JSON 파싱 오류" 응답으로 끝나던 이슈 수정. `_call_mail_api`에서 `json.loads` 실패 시 `strict=False` 폴백을 거치도록 변경하고, 둘 다 실패할 때만 raw 응답 head/tail 200자를 stderr에 남기도록 진단 로그 추가. `[TOOL_OUTPUT]` 풀 로깅 화이트리스트에 메일 6개 도구(inbox/sent/unread/search/folders/detail)를 추가하여 향후 동일 이슈가 재발해도 ToolMessage 전체가 운영 로그에 남도록 보강. 운영 로그상 `get_unread_mail`은 1.2초 만에 종료되는데도 LLM이 "특수 문자로 인한 파싱 오류"로 응답하던 증상이, 디버그 헤더가 정확히 300자라 [TOOL_OUTPUT] 트렁케이션에 의해 실제 에러 텍스트가 로그에 안 찍히던 부수 문제까지 동시에 해결 → [상세](docs/history/2026-04-28_MailJsonParseLenient.md)
