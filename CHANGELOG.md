...
## [2026-05-06] - Cron 자동 호출 인프라 + 사이드바 시각 구분 (Phase 3b)

- **추가** [agents/cron, agent-store/new, sidebar] 워크스페이스에 부착된 외부 Agent를 cron으로 자동 호출하고 결과를 새 채팅 세션 + 알림 deep-link로 통보. 사용 케이스: "매일 9시 인사이트 리포트 자동 생성". (1) **마이그레이션** `add_cron_session_metadata.sql` — `chat_sessions`에 `auto_generated`/`source_agent_id` 컬럼 + 인덱스 (수동 실행 필요). (2) **CronRunner** — `(workspace_uuid, agent_id)` 받아 self-contained 실행: workspace_agents의 `attached_by_user_id`로 cron user_id 결정, 새 `cron_<uuid>` 세션 발급, `external_agent_router.instantiate_worker_for_agent`로 Worker 생성, `triggers[0].prompt` 또는 default 프롬프트로 호출, 응답 chunks 수집 → `chat_log_service.save_chat_log` → `user_notifications` INSERT(`link_url=/chat/{session_id}`). 실패도 동일 알림(⚠ 실행 실패). (3) **AgentCronScheduler** — APScheduler `AsyncIOScheduler` 싱글턴, Job ID `cron_{ws}_{agent}`, `misfire_grace_time=600`. Hook 4개: `on_workspace_attach/detach`, `on_agent_manifest_changed`, `start/reload_all`. agent_service 5개 지점에서 hook 호출. `AGENT_CRON_ENABLED=false`로 차단. (4) **등록 폼** — `schedulePrompt` 필드 + `triggers[0].prompt` 매니페스트 직렬화. 분단위 cron 경고. (5) **사이드바 시각 구분** — `chat_log_service` SELECT가 `auto_generated`/`source_agent_id` 노출 → frontend `/api/history` route forward → `Chat` 타입 확장 → `groupChatsByDate`가 자동 세션을 별도 `autoGenerated` 그룹으로 분리 → 사이드바에 "🔄 자동 실행" 헤더(파란 톤) + ChatItem에 lucide `Clock` 아이콘. 일반 대화 흐름과 시각적으로 명확히 구분. 알림 인박스는 이미 `item.link` 기반 `router.push` 패턴이라 추가 wiring 불필요. → [상세](docs/history/2026-05-06_Cron_자동세션_Phase3b.md)

---

## [2026-05-06] - 워크스페이스 영속 파일 + MISO Workflow 출처 분리 (Phase 3a)

- **추가** [workspace, agents/miso_worker, agent-store/new] MISO Workflow 호출 시 파일 입력을 **채팅 첨부(일회성)** 와 **워크스페이스 영속(고정)** 두 출처로 분리. (1) **워크스페이스 원본 보존 인프라**: 기존엔 워크스페이스 업로드 시 ChromaDB 청크/임베딩만 저장되고 원본은 즉시 삭제(temp 파일 finally cleanup)되어 외부 호출 불가능. `data/workspace_uploads/{ws_uuid}/{file_id}/{filename}`에 명시적 보존(파일명 sanitize, file_id 격리), 삭제 시 통째 정리. ChromaDB 실패와 무관하게 RAG는 정상 동작하도록 non-fatal. `WorkspaceService.list_persisted_files_by_uuid` staticmethod로 워커가 DB 조회 없이 디스크 스캔만으로 영속 파일 목록 조회. (2) **MISO Worker 출처 토큰 분리**: 단일 파일은 `{{latest_file}}`/`{{file:..}}`(채팅) vs `{{workspace_file:..}}`(영속) — 명시 파일명. 다중 파일은 `{{recent_files[:N]}}`(채팅) vs `{{workspace_files[:N]}}`(영속). 토큰이 출처를 명시하므로 사용자 행동(첨부 여부)과 무관하게 매니페스트 의도대로 매핑. fuzzy fallback 도입 안 함 — 모호함은 운용 비용. (3) **등록 폼**: source 셀렉트에 `[채팅 첨부]`/`[워크스페이스]` 라벨 prefix로 출처 분리. 안내 박스에 두 출처의 사용 케이스 명시(사용자 직접 vs cron 자동). 향후 Phase 3b(cron 결과 라우팅 = 자동 세션 + 알림 deep-link)에서 영속 파일 토큰 그대로 재사용. → [상세](docs/history/2026-05-06_워크스페이스_영속파일_워크플로우호출.md)

---

## [2026-05-06] - MisoWorker Workflow 응답 추출 + Multi-row 입력 매핑 + 파일 자동 업로드

- **수정** [agents/miso_worker, agent-store/new] MISO Workflow Agent 운용을 위한 3개 보강을 한 묶음으로 진행. (1) **응답 추출 버그**: MISO 측 `outputs.answer`는 정상인데 채팅에는 `(워크플로우 출력이 비어있습니다)`만 뜨던 문제 — Workflow blocking 응답의 `outputs`가 최상위가 아닌 `data.data.outputs`에 중첩되는 게 원인. `_extract_answer`에 nested fallback 추가, chat 모드도 `data.data.answer` 보강. (2) **Multi-row 입력 매핑**: 단일 변수명 입력을 `[{name, type, source}, ...]` 다중 행 매핑으로 확장. 텍스트/문단/목록/숫자/단일 파일/**다중 파일** 6종 타입 지원(MISO 변수 유형 6종 풀커버: 텍스트/문단/목록/숫자/File/Array[File]). 타입에 따라 source 드롭다운이 동적 변경. 변수명 중복 검증, `_build_request_body` 비동기화 + list/dict 양 형태 호환(기존 등록 Agent 무수정 동작). (3) **파일 변수 자동 매핑 (단일 + 다중)**: ITSupportWorker의 `register_works_voc` 첨부 패턴 차용 — 워커가 `data/user_uploads/{date}/{user_id}/`에서 mtime 기준으로 파일을 찾아 `POST /ext/v1/files/upload`로 업로드 → `upload_file_id`(응답 wrapping 4개 위치 양보 매칭) → file ref. **단일 파일**은 mtime 최신 1개(`{{latest_file}}`), **다중 파일**은 시간 윈도우(`{{recent_files}}`, 기본 10분) 또는 N개 한정(`{{recent_files:3/5/10}}`) 모드 지원. 한 호출 최대 10개 안전 상한. 파일 미첨부 시 `FileMappingError` → 친절한 안내. ChatRequest 스키마 무변경. 신규 env: `MISO_FILE_UPLOAD_TIMEOUT`(120초)/`MISO_RECENT_FILES_WINDOW_SEC`(600초)/`MISO_RECENT_FILES_MAX`(10). → [상세](docs/history/2026-05-06_MisoWorker_Workflow_응답추출.md)

---

## [2026-05-04] - Agent Store 삭제 기능

- **추가** [agent-store/ui] 작성자/operator가 등록한 에이전트를 UI에서 직접 삭제할 수 있는 affordance 추가. 백엔드는 이미 `DELETE /v1/agents/{slug}` (soft delete, 작성자 또는 operator 권한)을 제공 중이었지만 프론트엔드에 UI가 없어 mock 카드 외에는 정리 수단이 없던 상태. (1) **에이전트 디테일 페이지** — `agent.isMine || isOperator` && `!isNative`일 때 헤더 액션 영역(설치/실행 버튼 옆)에 destructive outline `삭제` 버튼 표시. AlertDialog로 "이 작업은 되돌릴 수 없습니다" 확인 후 `agentApi.delete(slug)` 호출 → 성공 시 `/agent-store`로 라우팅. operator 판별은 `isOperatorUser(getUserId())`로 마운트 시 1회. (2) **카탈로그 그리드 (특히 My Creations 탭)** — `AgentCard`에 `onDelete?: (agent: Agent) => void` 옵셔널 prop 추가, `agent.isMine && !agent.isNative`일 때만 설치 버튼 옆에 작은 휴지통 아이콘 버튼 표시. 카드 클릭으로의 라우팅과 분리하기 위해 `e.stopPropagation()`. 삭제 트리거는 부모(`agent-store-content.tsx`)의 `handleDeleteRequest`로 위임 — `deleteTarget` 상태에 후보 보관, 카드/디테일과 별개로 store 레벨에서 단일 AlertDialog 운용. 확인 시 `agentApi.delete` → 성공 토스트 + 로컬 `agents` 상태에서 제거(서버 라운드트립 없이 즉시 반영). 삭제 진행 중 dialog dismiss 차단(`!deleting`). Native 카드는 안전망으로 `toast.info` 안내 후 dialog 띄우지 않음(백엔드도 거부). → [상세](docs/history/2026-05-04_AgentStore_삭제_기능.md)

---

- **수정** [agent-store/ui] 카탈로그 라우팅 화면 필터 재구성 — (1) 부서 카테고리 Select 제거 (사용자 요청), (2) 범위 필터에 Native 옵션 신설 — 에이전트의 effective scope를 `a.isNative ? "native" : a.visibility`로 산출해 Native seed Agent가 visibility 값과 무관하게 항상 `native` 버킷에 들어가도록 하여 "Native만 보기"·"Native 빼고 보기"를 한 셀렉트에서 처리, (3) 기능·범위 두 셀렉트를 단일 선택 `Select` → `DropdownMenu` + `DropdownMenuCheckboxItem` 멀티 체크로 전환. 트리거는 기존 SelectTrigger 룩앤필 유지(`asChild` + 직접 스타일된 button + ChevronDown), 0개 → "전체 X" / 1개 → 단일 라벨 / 2개+ → "X N개" 라벨 규칙. 체크 토글 시 메뉴가 닫히지 않도록 `onSelect preventDefault` 처리해 빠른 멀티 선택 가능. 상태 타입은 `Capability | "all"` → `Capability[]`, `Visibility | "all"` → `ScopeOption[]`로 변경하고 `hasFilters`/`handleResetFilters`도 `length > 0`/`[]` 기반으로 정리. 정렬 셀렉트는 단일 선택이라 `Select` 그대로. shadcn에 popover 컴포넌트가 없어 기존 `dropdown-menu.tsx` 재활용(의존성 추가 X), `DEPARTMENTS` mock 상수는 향후 재사용 여지로 보존만 하고 import만 제거. → [상세](docs/history/2026-05-04_AgentStore_필터_정비.md)

---

## [2026-05-04] - Agent Store 아이콘 리프레시

- **수정** [agent-store/ui] 카탈로그 카드가 "AI 자동 생성 placeholder" 같다는 사용자 피드백 반영. (1) Lucide `Sparkles`(별/스파클) → `Puzzle`(퍼즐 조각)로 fallback 및 iconMap 값 교체 — Sparkles는 ChatGPT/Notion AI/Copilot이 모두 default로 써서 클리셰화됐고, Agent Store의 "설치해 끼워 넣는 모듈" 메타포에는 Puzzle이 의미적으로 적합. `iconMap`의 키는 `"Sparkles"` 그대로 두고 값만 `Puzzle` 컴포넌트로 매핑해 백엔드/mock 데이터 contract 보존(마이그레이션 불필요). (2) 메인 아이콘 박스의 컬러 타일 배경(`${iconColor}20`) + `rounded-xl/2xl` 제거 → 카드 배경에 자연스럽게 녹아들어 아이콘 자체만 떠 보임. wrapper `<div>` 사이즈는 유지(레이아웃·정렬 보존), 박스 프레이밍이 사라진 만큼 아이콘 사이즈 한 단계씩 키움(h-5→h-6, h-8→h-10, h-4→h-5). 4개 surface(카드/디테일/피커/워크스페이스 탭) + empty-state CTA까지 일관 반영. 검증 우회로 Phosphor duotone PoC 시도했으나 사용자가 "구조 유지" 선회 → 패키지 제거하고 Lucide만으로 시각 처리. 동일한 iconMap 정의가 4곳에 중복돼 있어 향후 단일 모듈 추출 리팩토링 여지. → [상세](docs/history/2026-05-04_AgentStore_아이콘_리프레시.md)
