...
## [2026-04-24]
- **수정** [Calendar/Embed] 그룹웨어 위젯 경로에서 GOSSOcookie가 백엔드까지 전달되지 않아 `create_event`가 "서버에 연결할 수 없습니다"로 실패하던 이슈 수정 — 위젯 JS는 그룹웨어 origin의 `document.cookie`에서 `GOSSOcookie`를 추출해 iframe src에 `?gosso=` 파라미터로 붙여주지만, `middleware.ts` matcher가 `/embed`를 제외하고 `/embed/gw/page.tsx`도 `token/empno/sid`만 추출하여 `use-simple-chat.ts`의 쿠키 폴백이 빈 값을 받던 상태. 1차로 `useEffect` 쿠키 이식을 시도했으나 iframe context의 쿠키 쓰기/캐시 이슈로 여전히 미전달 확인(A2604011 재시도 로그도 `gosso → 없음`). 2차로 쿠키 경유를 우회하여 **URL → state → EmbedChat prop → useSimpleChat → fetch body** 직접 경로로 전환: `page.tsx`에서 URL의 `gosso`를 `useState`로 추출, `EmbedChat/useSimpleChat`에 `gossoCookie` prop 추가, fetch body에서 prop 값 우선 사용 후 기존 쿠키 폴백은 보존. 브라우저 캐시·SameSite 정책·iframe 쿠키 쓰기 차단에 무관하게 동작 → CalendarWorker가 사용자 본인 쿠키로 `/api/calendar/{id}/event/` POST 수행 → [상세](docs/history/2026-04-24_GW위젯_GOSSO_전달_수정.md)
