...
USER_MEMORY_KEY_FACTS_LIMIT = 100   # 최대 100개
...
# ============================================================
# 프롬프트 템플릿 - 사용자 메모리 정리(Consolidation)
# ============================================================
USER_FACTS_CONSOLIDATION_PROMPT = """당신은 사용자 프로필 정리 전문가입니다. 아래 사실 목록이 너무 많아졌으므로, 최대한 압축하여 갯수를 줄여주세요.

## 현재 사실 목록 ({current_count}개)
{all_facts}
...
### 적극 병합
- 의미가 유사하거나 중복된 사실은 하나로 통합
- 예: "React를 좋아함" + "프론트엔드 개발자" → "프론트엔드 개발자, React 선호"
- 예: "보고서는 한국어로" + "발표자료도 한국어로" → "문서 작성 시 한국어 선호"

### 적극 삭제
- 더 이상 유효하지 않거나 상충하는 정보 (최신 정보만 유지)
- 구체적 맥락 없이 모호한 사실
- 일회성이거나 가치가 낮은 정보

## 출력 형식
정리된 사실을 한 줄에 하나씩 출력하세요.
최대한 줄이되, 중요한 정보는 반드시 보존하세요.
번호, 기호, 설명 없이 사실 내용만 출력하세요.
...
class WorkspaceMemoryService:
    ...
    async def update_summary(
        ...
        try:
            ...
            new_summary = await self.bedrock.generate_text_haiku(
                prompt, caller="memory_ws_summary",
                user_id=user_id,
            )
    ...
    async def _extract_key_facts(
        ...
        try:
            ...
            response = await self.bedrock.generate_text_haiku(
                prompt, caller="memory_ws_facts", user_id=user_id,
            )
...
class UserMemoryService:
    ...
    async def get_user_memory(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            row = cursor.fetchone()
            print(f"[UserMemory] DB query for {user_id}: row={'found' if row else 'None'}")
            if not row:
                return None

            key_facts_json = row["key_facts"]
            last_updated = row["last_extracted_at"]
            print(f"[UserMemory] key_facts_json type={type(key_facts_json)}, length={len(str(key_facts_json)) if key_facts_json else 0}")

            # key_facts JSON 파싱
            key_facts = []
            if key_facts_json:
                try:
                    data = json.loads(key_facts_json) if isinstance(key_facts_json, str) else key_facts_json
                    # {"facts": [...]} 래퍼 또는 bare list [...] 둘 다 지원
                    facts_list = data.get("facts", []) if isinstance(data, dict) else data if isinstance(data, list) else []
                    key_facts = [f["content"] for f in facts_list if isinstance(f, dict) and "content" in f]
                    print(f"[UserMemory] Parsed {len(key_facts)} facts")
                except Exception as parse_err:
                    print(f"[UserMemory] JSON parse error: {parse_err}")

            if not key_facts:
                print(f"[UserMemory] No facts after parsing, returning None")
                return None
    ...
    async def _extract_user_facts(
        ...
        try:
            ...
            response = await self.bedrock.generate_text_haiku(
                prompt, caller="memory_user_facts", user_id=user_id,
            )
            ...
            # 초과 시 LLM 기반 정리 (공격적 압축)
            if len(all_facts) > USER_MEMORY_KEY_FACTS_LIMIT:
                all_facts = await self._consolidate_user_facts(all_facts, user_id=user_id)
    ...
    # --------------------------------------------------------
    # 사용자 facts 정리 (LLM 기반 압축)
    # --------------------------------------------------------
    async def _consolidate_user_facts(self, all_facts: List[Dict], user_id: str = None) -> List[Dict]:
        """
        LLM으로 facts를 공격적으로 압축.
        병합/삭제로 최대한 갯수를 줄인다.
        실패 시 FIFO 폴백.
        """
        try:
            ...
            prompt = USER_FACTS_CONSOLIDATION_PROMPT.format(
                current_count=len(all_facts),
                all_facts=facts_text
            )

            response = await self.bedrock.generate_text_haiku(
                prompt, caller="memory_user_consolidation", user_id=user_id,
            )
            ...
            for line in response.strip().split("\n"):
                ...
                if line.startswith("#") or line.startswith("("):
                    continue
                if any(kw in line for kw in ["없습니다", "정리된 사실", "출력"]):
                    continue
                consolidated.append(line)

            # 검증: 최소 1개, 최대 원본 이하
            if not consolidated:
                print(f"[UserMemory] Consolidation returned empty, falling back to FIFO")
                return all_facts[-USER_MEMORY_KEY_FACTS_LIMIT:]

            # 기존 메타데이터 보존 (원문 매칭 시)
            existing_content_map = {f["content"]: f for f in all_facts}
            now_iso = datetime.now().isoformat()

            result = []
            for content in consolidated:
                if content in existing_content_map:
                    result.append(existing_content_map[content])
                else:
                    # 병합/재작성된 fact → 새 타임스탬프
                    result.append({
                        "content": content,
                        "extracted_at": now_iso,
                    })

            print(f"[UserMemory] Consolidated {len(all_facts)} → {len(result)} facts")
            return result

        except Exception as e:
            print(f"[UserMemory] Consolidation failed, falling back to FIFO: {e}")
            return all_facts[-USER_MEMORY_KEY_FACTS_LIMIT:]
