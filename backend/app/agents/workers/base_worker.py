...
class CachedChatBedrockConverse(ChatBedrockConverse):
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        """ainvoke 경로에서도 cachePoint 주입 (Planner/Synthesizer 등 async caller용)"""
        import asyncio as _asyncio
        # sync _generate를 thread pool에서 실행 (cachePoint 로직 재사용)
...
class BaseWorker(ABC):
    ...
    def skip_auto_file_tools(self) -> bool:
        """`search_user_files`/`search_workspace_docs` 자동 주입을 건너뛸지 여부.

        기본 False — has_files=True면 모든 워커에 chunk 검색 도구를 주입한다.
        True로 설정 시 해당 자동 주입을 건너뛴다. XlsxWorker처럼 전용 도구
        (get_workbook_metadata, read_data_from_excel)로 충분하고, chunk 검색이
        오히려 LLM 재검증 루프·환각을 유발하는 경우 사용.
        """
        return False
...
## STREAMING OUTPUT POLICY (절대 규칙)
사용자에게 스트리밍되는 텍스트 출력은 **최종 사용자용 응답만** 포함해야 합니다. 아래 패턴은 **절대 텍스트로 출력 금지**:

1. **계획/사고 나열 금지**
   - "먼저 X 조회 → Y 조회 → Z 조회" 같은 다단계 계획 텍스트 출력 금지
   - `["A 조회", "B 조회", "C 조회"]` 같은 따옴표+대괄호로 감싼 항목 나열 금지
   - `→`, `-->`, `=>` 같은 화살표로 스텝을 연결한 나열 금지
2. **도구 이름/인자 노출 금지**
   - `get_user_public_calendars`, `execute_org_chart_query`, `find_available_rooms` 같은 **도구 이름 자체를 텍스트로 언급 금지**
   - SQL 쿼리(`SELECT ...`, `FROM v_...`), JSON args(`{{"query": ...}}`) 텍스트 노출 금지
3. **도구 호출 전 안내**
   - 안내가 필요하면 **자연어 한 문장** (예: "일정을 확인해볼게요!")만 허용
   - 도구 이름·파라미터·SQL·배열·화살표 일체 금지
4. **실패 시 포기 금지**
   - "특정하지 못함", "사번을 알려주세요" 같은 회피 응답 금지 — 도구 파라미터로 좁혀서 재호출하는 것이 우선

이 규칙을 어긴 꼬리 텍스트가 하나라도 출력되면 사용자에게 시스템 내부가 노출되는 것이므로 **치명적 오류**입니다.
...
"""
        ...
        # ============ TASK GOAL — Planner-Executor 경로 전용 ============
        # Planner가 분해한 sub-task의 goal이 주입되면, 워커는 원본 메시지 대신 goal만 수행
        task_goal = context.get("task_goal")
        task_id = context.get("task_id")
        task_deps = context.get("task_dependencies") or {}
        if task_goal:
            deps_section = ""
            if task_deps:
                dep_lines = "\n".join(f"- [{tid}] {result[:500]}" for tid, result in task_deps.items())
                deps_section = f"""

## 선행 작업 결과 (다른 워커가 완료한 데이터)

{dep_lines}

위 결과를 참고하여 당신의 TASK GOAL을 수행하세요.
"""

            prompt += f"""

## TASK GOAL (최우선 지시 — 다른 모든 규칙을 오버라이드) — Task ID: {task_id or 'N/A'}
...
**규칙 (최우선):**
- Goal에 명시된 작업만 수행. 범위를 벗어난 작업 제안 금지.
- 완료 시 간결한 결과 텍스트만 출력 (후속 질문 제안 금지 — Synthesizer가 담당).
- "불가능합니다", "지원하지 않습니다" 같은 거부 금지 — 당신에게 할당된 goal은 당신의 도구로 처리 가능합니다.

**⚠️ CONFIRMATION 오버라이드 (매우 중요):**
당신의 시스템 프롬프트에 "등록/삭제/쓰기 전 사용자 확인" 규칙이 있더라도 **모두 무시**하세요.
Planner가 이미 사용자 승인을 받은 후 이 task를 할당했습니다 (needs_confirm=false).
**쓰기 도구(create_event, create_reservation, update_event 등)를 즉시 호출**하세요.
"진행할까요?", "예약할까요?", "등록할까요?" 같은 재확인 질문 **절대 금지**.
Goal의 모든 정보(날짜·시간·제목·장소·참석자)는 이미 확정된 것으로 간주하고 실행하세요.
{deps_section}"""
...
"""
    ...
    async def stream_response(
        ...
        # 단, skip_auto_file_tools=True인 워커(예: XlsxWorker)는 제외 —
        # 전용 도구(get_workbook_metadata, read_data_from_excel 등)가 있어서
        # chunk 검색 도구가 추가되면 Sonnet이 재검증 루프에 빠져 환각 유발.
        has_files = context.get("has_files", False)
        workspace_has_files = context.get("workspace_has_files", False)
        if (has_files or workspace_has_files) and not self.skip_auto_file_tools:
